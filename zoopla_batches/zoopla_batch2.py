...
import requests
from district import postcode_batch2, borough_batch2
import pandas as pd
...
logging.basicConfig(filename='./logs/zoopla_batch2.log', filemode='a', format='%(levelname)s - %(asctime)s - %(message)s',level=logging.INFO)
...
def get_data(verbose=True):
    ...
    # scrape by postcode
    for code, town in zip(postcode_batch2, borough_batch2):
...
df.to_csv("data/zoopla_batch2.csv")
logging.info("Success! zoopla scraped data created with %d properties" % quantity)
end = time.time()
logging.info("Zoopla batch2 scrapping runtime: %d seconds" % (end-start))
