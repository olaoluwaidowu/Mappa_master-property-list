...
import requests
from district import postcode_batch6, borough_batch6
import pandas as pd
...
def get_data(verbose=True):
    ...
    # scrape by postcode
    for code, town in zip(postcode_batch6, borough_batch6):
...
df.to_csv("data/zoopla_batch6.csv")
logging.info("Success! zoopla scraped data created with %d properties" % quantity)
end = time.time()
logging.info("Zoopla batch6 scrapping runtime: %d seconds" % (end-start))
