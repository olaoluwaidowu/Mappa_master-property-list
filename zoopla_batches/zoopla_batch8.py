...
import requests
from district import postcode_batch8, borough_batch8
import pandas as pd
...
def get_data(verbose=True):
    ...
    # scrape by postcode
    for code, town in zip(postcode_batch8, borough_batch8):
...
df.to_csv("data/zoopla_batch8.csv")
logging.info("Success! zoopla scraped data created with %d properties" % quantity)
end = time.time()
logging.info("Zoopla batch8 scrapping runtime: %d seconds" % (end-start))
