...
import requests
from district import postcode_batch3, borough_batch3
import pandas as pd
...
logging.basicConfig(filename='./logs/zoopla_batch3458.log', filemode='a', format='%(levelname)s - %(asctime)s - %(message)s',level=logging.INFO)
...
def get_data(verbose=True):
    ...
    # scrape by postcode
    for code, town in zip(postcode_batch3, borough_batch3):
...
df.to_csv("data/zoopla_batch3.csv")
logging.info("Success! zoopla scraped data created with %d properties" % quantity)
end = time.time()
logging.info("Zoopla batch3 scrapping runtime: %d seconds" % (end-start))
