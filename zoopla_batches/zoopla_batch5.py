...
import requests
from district import postcode_batch5, borough_batch5
import pandas as pd
...
def get_data(verbose=True):
    ...
    # scrape by postcode
    for code, town in zip(postcode_batch5, borough_batch5):
...
df.to_csv("data/zoopla_batch5.csv")
logging.info("Success! zoopla scraped data created with %d properties" % quantity)
end = time.time()
logging.info("Zoopla batch5 scrapping runtime: %d seconds" % (end-start))
