...
import requests
from district import postcode_batch7, borough_batch7
import pandas as pd
...
def get_data(verbose=True):
    ...
    # scrape by postcode
    for code, town in zip(postcode_batch7, borough_batch7):
...
df.to_csv("data/zoopla_batch7.csv")
logging.info("Success! zoopla scraped data created with %d properties" % quantity)
end = time.time()
logging.info("Zoopla batch7 scrapping runtime: %d seconds" % (end-start))
