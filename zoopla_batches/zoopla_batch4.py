...
import requests
from district import postcode_batch4, borough_batch4
import pandas as pd
...
def get_data(verbose=True):
    ...
    # scrape by postcode
    for code, town in zip(postcode_batch4, borough_batch4):
...
df.to_csv("data/zoopla_batch4.csv")
logging.info("Success! zoopla scraped data created with %d properties" % quantity)
end = time.time()
logging.info("Zoopla batch4 scrapping runtime: %d seconds" % (end-start))
